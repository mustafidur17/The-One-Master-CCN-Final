...
public class Settings {
	...
	/**
	 * Optional comma-separated list of run-array setting names that should
	 * be combined as a Cartesian product in batch mode. Settings not listed
	 * here keep the original THE ONE runIndex % arrayLength behavior.
	 */
	public static final String CARTESIAN_RUN_SETTINGS_S =
			"Settings.cartesianRunSettings";
	...
	public String getSetting(String name) {
		...
		if (value != null) { // found value, check if run setting can be parsed
			value = parseRunSetting(fullPropName, value.trim());
		}
	 ...
	 */
	private static String parseRunSetting(String fullPropName, String value) {
		...
		int arrIndex = runIndex % valueArr.length; // original THE ONE behavior

		String cartesianList = props.getProperty(CARTESIAN_RUN_SETTINGS_S);
		if (cartesianList != null && cartesianList.trim().length() > 0) {
			String[] dimensions = cartesianList.split(",");
			int dimensionIndex = -1;

			for (int i = 0; i < dimensions.length; i++) {
				if (dimensions[i].trim().equals(fullPropName)) {
					dimensionIndex = i;
					break;
				}
			}
			...
			if (dimensionIndex >= 0) {
				...
				/* Settings later in the list change faster. */
				for (int i = dimensionIndex + 1; i < dimensions.length; i++) {
					String dimName = dimensions[i].trim();
					String rawDimValue = props.getProperty(dimName);

					if (rawDimValue == null ||
						!rawDimValue.startsWith(RUN_ARRAY_START) ||
						!rawDimValue.endsWith(RUN_ARRAY_END)) {
						throw new SettingsError("Cartesian run setting '" +
								dimName + "' must exist and use [a;b;c] syntax");
					}

					String dimBody = rawDimValue.substring(1,
							rawDimValue.length() - 1);
					String[] dimValues = dimBody.split(RUN_ARRAY_DELIM);
					stride *= dimValues.length;
				}

				arrIndex = (int)((runIndex / stride) % valueArr.length);
			}
		}

		return valueArr[arrIndex].trim();
	}
